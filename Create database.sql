if exists(select * from sys.objects where name='v_db_employee_list')
	drop view v_db_employee_list
if exists(select * from sys.objects where name='v_db_manager_list')
	drop view v_db_manager_list
if exists(select * from sys.objects where name='db_vacation_requests')
	drop table db_vacation_requests
if exists(select * from sys.objects where name='db_workplace_accidents')
	drop table db_workplace_accidents
if exists(select * from sys.objects where name='db_employees')
	drop table db_employees
if exists(select * from sys.objects where name='db_departments_lookup')
	drop table db_departments_lookup
GO


CREATE TABLE [dbo].[db_departments_lookup](
	[department_id] [varchar](20) NOT NULL,
 CONSTRAINT [PK_db_departments_lookup_department_id] PRIMARY KEY ([department_id])
 )
...
CREATE TABLE [dbo].[db_employees](
 ...
 CONSTRAINT [PK_db_employees_employee_id] PRIMARY KEY ([employee_id] ),
 constraint [fk_db_employees_employee_department] FOREIGN KEY (employee_department) REFERENCES db_departments_lookup(department_id)
 )

create table db_workplace_accidents
(
	...
	constraint pk_db_workplace_accidents_accident_id 
		primary key(accident_id),
	constraint fk_db_workplace_accidents_accident_reported_by_employee_id
		foreign key (accident_reported_by_employee_id) references db_employees(employee_id),
	constraint fk_db_workplace_accidents_accident_acknowledged_by_store_mgr_employee_id
		foreign key (accident_acknowledged_by_store_mgr_employee_id) references db_employees(employee_id),
	constraint fk_db_workplace_accidents_accident_department
		foreign key (accident_department) references db_departments_lookup(department_id)
)
go
create table db_vacation_requests
(
	...
	constraint pk_db_vacation_requests_request_id primary key (request_id),
	constraint fk_db_vacation_requests_request_employee_id 
		foreign key (request_employee_id) references db_employees(employee_id),
	constraint fk_db_vacation_requests_request_approved_employee_id
			foreign key (request_approved_employee_id) references db_employees(employee_id)
)
go
create view v_db_employee_list as
	select employee_id, employee_firstname + ' ' + employee_lastname as employee_name
		from db_employees
go
create view v_db_manager_list as
	select distinct e.* from v_db_employee_list e join db_employees m on e.employee_id = m.employee_supervisor_id 
go

INSERT [dbo].[db_departments_lookup] ([department_id]) VALUES (N'Clothing')
INSERT [dbo].[db_departments_lookup] ([department_id]) VALUES (N'Customer Service')
INSERT [dbo].[db_departments_lookup] ([department_id]) VALUES (N'Electronics')
INSERT [dbo].[db_departments_lookup] ([department_id]) VALUES (N'Hardware')
INSERT [dbo].[db_departments_lookup] ([department_id]) VALUES (N'Housewares')
INSERT [dbo].[db_departments_lookup] ([department_id]) VALUES (N'Sporting Goods')
GO

INSERT [dbo].[db_employees] ([employee_id], [employee_ssn], [employee_lastname], [employee_firstname], [employee_jobtitle], [employee_department], [employee_birthdate], [employee_hiredate], [employee_termdate], [employee_hourlywage], [employee_fulltime], [employee_supervisor_id]) VALUES (1, N'111220001', N'Photo', N'Arial', N'Sales Associate', N'Electronics', CAST(N'1982-01-12T00:00:00.000' AS DateTime), CAST(N'2011-07-05T00:00:00.000' AS DateTime), NULL, 15.2830, 0, 5)
...
GO
INSERT [dbo].db_vacation_requests ([request_name],[request_employee_id],[request_start_date],[request_number_of_days],[request_approved_employee_id], [request_approved_date]) VALUES (N'disney',1,2020-10-30,8,null,null)
INSERT [dbo].db_vacation_requests ([request_name],[request_employee_id],[request_start_date],[request_number_of_days],[request_approved_employee_id], [request_approved_date]) VALUES (N'foo',5,2020-10-14,8,null,null)
INSERT [dbo].db_vacation_requests ([request_name],[request_employee_id],[request_start_date],[request_number_of_days],[request_approved_employee_id], [request_approved_date]) VALUES (N'demo',2,2020-10-13,4,null,null)
select * from dbo.db_vacation_requests
